...
- **OPEN-5** (resolved by issue #9, 2026-05-08): The original framing of OPEN-5 — "document the three-tier branch model (release → feature → task) in governance.md" — was superseded during v0.0.2 process hardening. Lived experience clarified that the policy is about *what gets pushed* (only release branches), not about counting tiers. `docs/governance.md` now describes the remote-only-release-branches policy with explicit local/remote distinction. The three-tier framing has been retired.
