...
# `from maestro.X import ...` imports below resolve naturally.
# ============================================================

from maestro import paths as _maestro_paths  # noqa: F401
from maestro.env_loader import load_credentials
...
def run_stdio() -> None:
    """Sync wrapper around the async `main()` coroutine.

    Used by:
      - `maestro serve` (via maestro.cli.main._cmd_serve)
      - `bootstrap/maestro_server.py` deprecation shim
      - Direct `python -m maestro.mcp_server` invocation

    KeyboardInterrupt cleanly exits with code 0; any other exception
    propagates so the caller (CLI) can decide policy.
    """
    try:
        asyncio.run(main())
